defmodule Contact.UserFromAuthTest do
  use Contact.ModelCase
  ...
  alias Contact.UserFromAuth
  alias Ueberauth.Auth
  alias Ueberauth.Auth.Credentials
  alias Ueberauth.Auth.Info

  @name "Jake Humphrey"
  @email "humphreyja@mnstate.edu"
  @uid "humphreyja"
  @provider :github
  @token "test-token"
  ...
  test "it returns an error if the user is not the current user", %{auth: auth} do
    {:ok, current_user} = User.registration_changeset(%User{}, %{email: "foo@bar.com", name: @name}) |> Repo.insert
    {:ok, user} = User.registration_changeset(%User{}, %{email: @email, name: @name}) |> Repo.insert
    {:ok, _authorization} = Authorization.changeset(
      Ecto.Model.build(user, :authorizations),
      %{
